...
printf("loop_start_id %d\n", ++global_loop_id);    for(int i=s;i<=e;i++)
    {
printf("line = %d\n",__LINE__);        ara[i] = s ;
    }printf("loop_end_id %d\n", global_loop_id--);
printf("loop_start_id %d\n", ++global_loop_id);    for(int i=s;i<=e;i++)
    {
printf("line = %d\n",__LINE__);        printf("%d\n",ara[i]);
    }printf("loop_end_id %d\n", global_loop_id--);
...
printf("line = %d\n",__LINE__);    int temp = 0 ;
printf("loop_start_id %d\n", ++global_loop_id);    for(int i=0;i<5;i++)
    {
printf("loop_start_id %d\n", ++global_loop_id);        for(int j=0;j<5;j++)
        {
...
printf("line = %d\n",__LINE__);            testAra[i] = temp ;
    ...
    //H2//
printf("loop_start_id %d\n", ++global_loop_id);    for(int i=0;i<3;i++)
    {
printf("loop_start_id %d\n", ++global_loop_id);        for(int j=0;j<3;j++)
        {
printf("loop_start_id %d\n", ++global_loop_id);            for(int k=0;k<3;k++)
            {
